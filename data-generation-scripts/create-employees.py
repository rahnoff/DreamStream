...
def create_employees_csv() -> None:
    ...
    edited_ats: list[str] = [datetime.datetime.now().astimezone().isoformat().replace('T', ' ') for edited_at in range(1000000)]
    emails: list[str] = [fake.unique.email() for email in range(1000000)]
    first_names: list[str] = [fake.first_name() for first_name in range(1000000)]
    last_names: list[str] = [fake.last_name() for last_name in range(1000000)]
