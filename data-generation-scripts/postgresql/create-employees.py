...
def create_employees_csv() -> None:
    ...
    ids: list[str] = [fake.uuid4() for id in range(2000000)]
    created_ats: list[str] = [datetime.datetime.now().astimezone().isoformat().replace('T', ' ') for created_at in range(2000000)]
    edited_ats: list[str] = [datetime.datetime.now().astimezone().isoformat().replace('T', ' ') for edited_at in range(2000000)]
    emails: list[str] = [fake.unique.email() for email in range(2000000)]
    first_names: list[str] = [fake.first_name() for first_name in range(2000000)]
    last_names: list[str] = [fake.last_name() for last_name in range(2000000)]
